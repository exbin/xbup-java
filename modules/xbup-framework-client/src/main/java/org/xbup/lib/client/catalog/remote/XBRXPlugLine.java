...
public class XBRXPlugLine implements XBCXPlugLine {
    ...
    public XBCXPlugin getPlugin() {
        try {
            XBCatalogServiceMessage message = client.executeProcedure(XBServiceClient.LINEPLUGIN_PLUGIN_PROCEDURE);
            ...
            Logger.getLogger(XBRXPlugLine.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public Long getLineIndex() {
        try {
            XBCatalogServiceMessage message = client.executeProcedure(XBServiceClient.LINEINDEX_PLUGIN_PROCEDURE);
            XBListener listener = message.getXBOutput();
            listener.attribXB(new UBNat32(getId()));
            listener.endXB();
            XBMatchingProvider checker = message.getXBInput();
