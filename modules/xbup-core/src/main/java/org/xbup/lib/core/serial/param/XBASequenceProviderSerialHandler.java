 ...
 */
public class XBASequenceProviderSerialHandler implements XBPSequenceSerialHandler, XBASequenceInputSerialHandler, XBASerialSequenceable, XBTTokenInputSerialHandler {
    ...
    public XBASequenceProviderSerialHandler() {
        provider = new XBPProviderSerialHandler();
    }

    public XBASequenceProviderSerialHandler(XBPReadSerialHandler childHandler) {
        provider = new XBPProviderSerialHandler(childHandler);
        ...
        for (XBSerialSequenceItem seqItem : sequence.getItems()) {
            ...
            switch (seqItem.getSequenceOp()) {
                case JOIN: {
                    ...
                    } else if (item instanceof XBPSerializable) {
                        XBJoinInputStream joinSerial = new XBJoinInputStream(handler);
                        // TODO ((XBTChildSerializable) item).serializeFromXB(joinSerial);
    ...
    private class XBJoinInputStream implements XBPInputSerialHandler {
